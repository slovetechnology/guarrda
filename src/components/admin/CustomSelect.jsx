...
function CustomSelect({ Options, error, LoadContent, defaultValue, contentValue }) {
    ...
    return (
        <div className="relative">
            ...
            <Menu shadow="md" classNames='relative p-0 mt-20' width={"25rem"} opened={opened} onChange={setOpened}>
                <Menu.Target>
                    <div>
                        <div className={`border ${error ? 'border-red-500' : 'border-zinc-300'} rounded-md grid grid-cols-2 items-center py-3 px-2 cursor-pointer`}>
                            {content?.short ? <div className='flex items-center gap-2'>  <img src={content.image} alt="" className="size-7" /> {content.short} </div> :
                    ...
                    <div className="h-[12.5rem] overflow-y-auto scrolls">
                        {values.map((item, index) => (
                            <div onClick={() => ManageCurrent(item)} key={index} className="py-1.5 px-2 hover:bg-zinc-100 cursor-pointer rounded-md flex items-center gap-2">
                                <img src={item.image} alt="" className="size-9" /> {item.short}
                            </div>
                        ))}
                    </div>
                </Menu.Dropdown>
